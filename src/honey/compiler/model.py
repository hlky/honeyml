...
from honey.compiler.dtype import dtype_str_to_enum
from honey.utils.misc import is_linux, is_windows
from honey.utils.torch_utils import torch_dtype_to_string, write_tensor_binary
...
class HoneyMemcpyKind(enum.Enum):
    HostToDevice = 0
    DeviceToHost = 1
    DeviceToDevice = 2


class HoneyAllocatorKind(enum.Enum):
    DEFAULT = 0
    TRACKING = 1


class HoneyData(NamedTuple):
    """
...
class _HoneyShape(ctypes.Structure):
    _fields_ = [
...
class _CFormatHoneyData(ctypes.Structure):
    _fields_ = [
...
def torch_to_honey_data(tensor: TorchTensor) -> HoneyData:
    """
    Convert a torch Tensor to a HoneyData.
    """
    return HoneyData(
...
def _convert_tensor_args(params: Union[List[TorchTensor], Dict[str, TorchTensor]]):
    ...
    if isinstance(params, dict):
        result = {name: torch_to_honey_data(x) for name, x in params.items()}
    else:
        result = [torch_to_honey_data(x) for x in params]
    return result
...
class Model:
    class _DLLWrapper:
        ...
        def __getattr__(self, name):
            if not self.is_open:
                raise RuntimeError(f"Cannot use closed Honey library: {self.lib_path}")
    ...
    def __init__(
        ...
        """
        # Set of pointers allocated with numpy_to_honey_data.
        # If the user forgets to free their data, we use this to
        # avoid leaking memory.
        self._allocated_honey_data = set()
    ...
    def close(self):
        ...
        for ptr in list(self._allocated_honey_data):
        ...
        if hasattr(self, "DLL"):
            if self.handle:
                self.DLL.HoneyModelContainerDelete(self.handle)
                self.handle = ctypes.c_void_p()

            if self.allocator_handle:
                self.DLL.HoneyAllocatorDelete(self.allocator_handle)
                self.allocator_handle = ctypes.c_void_p()
    ...
    def _convert_single_param_to_c_format(self, param: HoneyData) -> _CFormatHoneyData:
        ...
        c_shape = _HoneyShape(c_shape_data, ctypes.c_size_t(len(shape)))
        c_dtype = dtype_str_to_enum(dtype)
        return _CFormatHoneyData(c_pointer, c_shape, c_dtype)

    def _convert_params_to_c_format(self, params: List[HoneyData]):
        c_params = (_CFormatHoneyData * len(params))()
    ...
    def _make_honey_outputs(
        self, outputs: List[HoneyData], c_output_shapes
    ) -> Dict[str, HoneyData]:
        output_shapes = []
    ...
    def _run_impl(
        self,
        inputs: Union[Dict[str, HoneyData], List[HoneyData]],
        outputs: Union[Dict[str, HoneyData], List[HoneyData]],
        ...
        if not outputs_on_host:
            self.DLL.HoneyModelContainerRun(
                self.handle,
                c_inputs,
            ...
            self.DLL.HoneyModelContainerRunWithOutputsOnHost(
                self.handle,
                c_inputs,
                ctypes.c_size_t(len(inputs)),
                c_outputs,
        ...
        return self._make_honey_outputs(outputs, c_output_shapes_out)
    ...
    def run(
        ...
        ----------
        inputs: Union[Dict[str, HoneyData], List[HoneyData]]
            The inputs to use. HoneyData is a named tuple containing
            ...
            This parameter can also be a dictionary (name -> HoneyData).
        outputs: Union[Dict[str, HoneyData], List[HoneyData]]
            The outputs to use. Similar to inputs, can either be a list of ordered
            outputs, or a dictionary (output name -> HoneyData).
        ...
        Returns
        -------
        HoneyDatas with output shapes that are computed by shape inference. This may not be
    ...
    def profile(
        ...
        self.DLL.HoneyModelContainerProfile(
            self.handle,
            c_inputs,
            ctypes.c_size_t(len(inputs)),
            c_outputs,
    ...
    def _interpret_tensors_as_shapes(
        self,
        outputs: Union[List[TorchTensor], Dict[str, TorchTensor]],
        outputs_honey: Dict[str, HoneyData],
    ) -> Dict[str, TorchTensor]:
        if isinstance(outputs, dict):
            return {
                name: _reshape_tensor(tensor, outputs_honey[name].shape)
                for name, tensor in outputs.items()
            }
        else:
            return {
                name: _reshape_tensor(outputs[idx], outputs_honey[name].shape)
                for name, idx in self._output_name_to_index.items()
            }
    ...
    def run_with_tensors(
        ...
        return self._interpret_tensors_as_shapes(outputs, outputs_honey)
    ...
    def benchmark(
    ...
    ) -> Tuple[float, float, Dict[str, HoneyData]]:
        """
        ...
        for _ in range(repeat):
            self.DLL.HoneyModelContainerBenchmark(
                self.handle,
                c_inputs,
                ctypes.c_size_t(len(inputs)),
                c_outputs,
        ...
        return (mean, std, self._make_honey_outputs(outputs, c_output_shapes_out))
    ...
    def benchmark_with_tensors(
        ...
        mean, std, honey_outputs = self.benchmark(
            _convert_tensor_args(inputs),
            _convert_tensor_args(outputs),
            stream_ptr,
            graph_mode,
        ...
        return (mean, std, self._interpret_tensors_as_shapes(outputs, honey_outputs))
    ...
    def get_required_memory(self):
        ...
        self.DLL.HoneyModelContainerGetRequiredMemory(
    ...
    def set_constant(self, name: str, tensor: HoneyData):
        """
        ...
        self.DLL.HoneyModelContainerSetConstant(
            self.handle, c_name, ctypes.byref(c_tensor)
        )

    def set_many_constants(self, tensors: Dict[str, HoneyData]):
        """
    ...
    def set_double_buffer_constant(
        ...
        c_name = ctypes.c_char_p(b_name)
        c_tensor = self._convert_single_param_to_c_format(tensor)
        self.DLL.HoneyModelContainerSetDoubleBufferConstant(
            self.handle, ctypes.c_void_p(stream_ptr), c_name, ctypes.byref(c_tensor)
        )

    def set_many_double_buffer_constants(
        self, tensors: Dict[str, HoneyData], stream_ptr: Optional[int] = None
    ):
        """
        Bulk set many constants at once. More efficient than set_constant()
        since it only has to acquire the lock once.
        ...
        num_tensors = ctypes.c_size_t(len(tensors))
        self.DLL.HoneyModelContainerSetManyDoubleBufferConstants(
    ...
    def set_many_constants_with_tensors(self, tensors: Dict[str, TorchTensor]):
        ...
        for name, tensor in tensors.items():
            ...
            honey_tensors[name] = torch_to_honey_data(tensor)
        self.set_many_constants(honey_tensors)
    ...
    def set_double_buffer_constant_with_tensor(
        ...
        if not tensor.is_contiguous() or not tensor.is_cuda:
            raise ValueError(f"Constant {name} must be contiguous and on the GPU.")
        self.torch_constant_tensors[name] = tensor
        self.set_double_buffer_constant(name, torch_to_honey_data(tensor), stream_ptr)
    ...
    def set_many_double_buffer_constants_with_tensors(
        ...
        for name, tensor in tensors.items():
            ...
            self.torch_constant_tensors[name] = tensor
            honey_tensors[name] = torch_to_honey_data(tensor)
        self.set_many_double_buffer_constants(honey_tensors, stream_ptr)
    ...
    def set_constant_with_tensor(self, name: str, tensor: TorchTensor):
        ...
        if not tensor.is_contiguous() or not tensor.is_cuda:
            raise ValueError(f"Constant {name} must be contiguous and on the GPU.")
        self.torch_constant_tensors[name] = tensor
        self.set_constant(name, torch_to_honey_data(tensor))
    ...
    def get_input_maximum_shape(self, input_idx_or_name: Union[int, str]) -> List[int]:
        ...
        raw_shape = Shape()
        self.DLL.HoneyModelContainerGetMaximumInputShape(
    ...
    def get_input_dtype(self, index):
        ...
        """
        input = ctypes.c_int()
        self.DLL.HoneyModelContainerGetInputDtype(
    ...
    def get_output_maximum_shape(
        ...
        raw_shape = Shape()
        self.DLL.HoneyModelContainerGetMaximumOutputShape(
    ...
    def get_output_dtype(self, index):
        ...
        """
        output = ctypes.c_int()
        self.DLL.HoneyModelContainerGetOutputDtype(
    ...
    def free_gpu_memory(
        ...
        third-party libraries like PyTorch or pycuda are not available.
        """
        if ptr in self._allocated_honey_data:
            self._allocated_honey_data.remove(ptr)

        self.DLL.HoneyDeviceFree(
    ...
    def memcpy(
        ...
        specified by the HoneyMemcpyKind enum.
        """
        self.DLL.HoneyMemcpy(
            ctypes.c_void_p(dst),
            ctypes.c_void_p(src),
            ctypes.c_size_t(count),
    ...
    def get_num_runtimes(self) -> int:
        ...
        """
        out = ctypes.c_size_t()
        self.DLL.HoneyModelContainerGetNumRuntimes(self.handle, ctypes.byref(out))
        return out.value

    def numpy_to_honey_data(
        self, arr: np.ndarray, stream_ptr: Optional[int] = None, sync: bool = True
    ) -> HoneyData:
        """
        Convert a numpy array to Honey-usable data. Mallocs and copies
        on the given stream.
        ...
        self._allocated_honey_data.add(gpu_mem)
        self.memcpy(
            gpu_mem,
        ...
        return HoneyData(gpu_mem, shape, dtype)

    def honey_data_to_numpy(
        self,
        honey_data: HoneyData,
        stream_ptr: Optional[int] = None,
        sync: bool = True,
    ) -> np.ndarray:
        """
        Create numpy array from an HoneyData.
        Copies on the given stream.
        """
        arr = np.empty(honey_data.shape, dtype=honey_data.dtype)
        self.memcpy(
            arr.ctypes._data.value,
            honey_data.data_ptr,
            arr.nbytes,
    ...
    def fold_constants(
        ...
        if double_buffer:
            self.DLL.HoneyModelContainerFoldConstantsInDoubleBuffer(
                self.handle,
                ctypes.c_void_p(stream_ptr),
                ctypes.c_bool(sync),
            )
        else:
            self.DLL.HoneyModelContainerFoldConstants(
                self.handle,
                ctypes.c_void_p(stream_ptr),
                ctypes.c_bool(sync),
            )
    ...
    def swap_constants(self):
        self.DLL.HoneyModelContainerSwapConstants(self.handle)
    ...
    def _get_constant_names_impl(
        ...
        self.DLL.HoneyModelContainerGetNumConstants(
            self.handle,
        ...
        self.DLL.HoneyModelContainerGetConstantNames(
