...
class conv2d_bias_silu(conv2d_bias_activation):
    ...
    Applies a 2D convolution on input in shape (N, H, W, C_in), adds a bias in shape (C_out), performs silu and produces output in shape (N, H_out, W_out, C_out). N is batch size, H, W are the height and width of the input images in pixels, and C is the number of channels.
    ...
    .. code-block:: python
        ...
        OP = honey.compiler.ops.conv2d_bias_silu(stride=1, pad=1, dilate=1)
        Result_honey = OP(X, W, B)

    .. highlight:: python
    .. code-block:: python

        X_pt = NHWC2NCHW(X_honey)
        W_pt = NHWC2NCHW(W_honey)
        B_pt = NHWC2NCHW(B_honey)

        Y = torch.nn.functional.conv2d(X_pt, W_pt, bias=B_pt)
        Result_pt = torch.nn.functional.silu(Y)
        Result_honey = NCHW2NHWC(Result_pt)
    """
