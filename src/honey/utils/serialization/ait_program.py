...
def convert_to_honey_const(const):
    if isinstance(const, bytes):
        ...
        raise RuntimeError(f"Unknown type ({type(const)}) to convert to Honey Tensor")


class HoneyBasicProgram:
    def __init__(self):
        """
    ...
    def set_constants(self, constants: Dict[str, Any]):
        ...
        for k, v in constants.items():
            getattr(self, k)._bind_data(convert_to_honey_const(v))
    ...
    def model(self) -> Union[Tensor, Tuple[Tensor]]:
        """
        This function defines the Honey program.
