...
def enable_cuda_lto() -> bool:
    ...
    Default value is "0".
    """
    return os.getenv("Honey_ENABLE_CUDA_LTO", "0") == "1"
...
def nvcc_ccbin() -> str:
    ...
    """
    return os.getenv("Honey_NVCC_CCBIN", "")
...
def time_compilation() -> bool:
    ...
    """
    return os.getenv("Honey_TIME_COMPILATION", "0") == "1"
...
def shorten_tensor_names_for_plots() -> bool:
    ...
    """
    return os.getenv("Honey_PLOT_SHORTEN_TENSOR_NAMES", "0") == "1"


def honey_build_cache_dir() -> Optional[str]:
    """
    When set to a non-empty string, cache the build artifacts
    below this directory for significantly faster builds.

    See honey.backend.build_cache

    Returns:
        Optional[str]: Value of Honey_BUILD_CACHE_DIR environment variable,
        or None if not set.
    """
    return os.environ.get("Honey_BUILD_CACHE_DIR", None)


def honey_build_cache_skip_percentage() -> int:
    """
    When set to a non-empty string, and if Honey_BUILD_CACHE_DIR
    is set, the build cache will be skipped randomly with
    a probability correspinding to the specified percentage

    Returns:
        int: Integer value of Honey_BUILD_CACHE_SKIP_PERCENTAGE environment variable,
        or 5 if not set.
    """
    return int(os.environ.get("Honey_BUILD_CACHE_SKIP_PERCENTAGE", "30"))


def honey_build_cache_skip_profiler() -> bool:
    """
    boolean value of Honey_BUILD_CACHE_SKIP_PROFILER environment variable.
    ...
    in all other cases.
    """
    ret = os.environ.get("Honey_BUILD_CACHE_SKIP_PROFILER", "1")
    if ret is None or ret == "" or ret == "0" or ret.lower() == "false":
        return False
    return True


def honey_build_cache_max_mb() -> int:
    """
    boolean value of Honey_BUILD_CACHE_MAX_MB environment variable.
    ...
    """
    return int(os.environ.get("Honey_BUILD_CACHE_MAX_MB", "30"))
...
def allow_cutlass_sm90_kernels() -> bool:
    ...
    return (
        ...
        or os.getenv("Honey_ALLOW_CUTLASS_SM90_KERNELS", "0") == "1"
    )
...
def force_cutlass_sm90_kernels() -> bool:
    ...
    """
    return os.getenv("Honey_FORCE_CUTLASS_SM90_KERNELS", "0") == "1"
...
def multistream_mode() -> int:
    ...
    return int(os.getenv("Honey_MULTISTREAM_MODE", "0"))
...
def multistream_additional_streams() -> int:
    ...
    This option is independent from Honey_MULTISTREAM_MAX_MEM_PARALLEL_OPS.

    For example, say, there are 100 ops that can be run in parallel.

    Example 1: Honey_MULTISTREAM_EXTRA_STREAMS=4 and Honey_MULTISTREAM_MAX_MEM_PARALLEL_OPS=100.
    ...
    Example 2: Honey_MULTISTREAM_EXTRA_STREAMS=4 and Honey_MULTISTREAM_MAX_MEM_PARALLEL_OPS=5.
    In this case 5 streams will be used (1 base and 4 extra),
    ...
    """
    return int(os.getenv("Honey_MULTISTREAM_EXTRA_STREAMS", "4"))
...
def multistream_max_mem_parallel_ops() -> int:
    ...
    This option is independent from Honey_MULTISTREAM_EXTRA_STREAMS.
    ...
    return int(os.getenv("Honey_MULTISTREAM_MAX_MEM_PARALLEL_OPS", "99999999"))
...
def allocation_mode() -> int:
    ...
    Default: 0.
    """
    return int(os.getenv("Honey_ALLOCATION_MODE", "0"))
...
def is_cmake_compilation() -> bool:
    ...
    return os.getenv("Honey_USE_CMAKE_COMPILATION", "1" if is_windows() else "0") == "1"


def enable_standalone_exe_generation() -> bool:
    """
    Whether to generate standalone binaries for Honey build directories.
    Defaults to False.
    """
    return os.getenv("Honey_ENABLE_STANDALONE", "0") == "1"
...
def get_cuda_nvcc_debug_level():
    ...
    backed by env var Honey_CUDA_DEBUG_LEVEL. Which may either be a string
    ...
    """
    level = os.getenv("Honey_CUDA_DEBUG_LEVEL", "0")
    return level
