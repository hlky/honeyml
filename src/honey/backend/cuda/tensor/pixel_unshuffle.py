...
from honey.backend.common import pixel_unshuffle_common
...
@registry.reg("cuda.pixel_unshuffle.gen_function")
def gen_function(
    func_attrs,
    template_path,
):
...
@registry.reg("cuda.pixel_unshuffle.func_decl")
def pixel_unshuffle_gen_function_decl(func_attrs):
    return pixel_unshuffle_common.gen_function_decl(func_attrs, backend_spec=CUDASpec())


@registry.reg("cuda.pixel_unshuffle.func_call")
