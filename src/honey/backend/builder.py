...
from honey.backend import build_cache
from honey.backend.build_cache_base import write_binhash_file

from honey.backend.target import Target
from honey.backend.task_runner import BaseRunner, Task

from honey.utils import environ

from honey.utils.debug_settings import HoneyDebugSettings

from honey.utils.environ import is_cmake_compilation
...
class Builder:
    ...
    def __init__(self, n_jobs: int = -1, timeout: int = 180) -> None:
        ...
        self._do_trace = os.environ.get("Honey_TRACE_MAKE", False)
...
"""
        ...
        obj_files = []
        # * standalone.cu is an Honey internal file that is used for generating
...
"""
    ...
    def make_profilers(self, generated_profilers, workdir):
        ...
        _run_make_cmds(
            ...
            self._timeout,
            build_dir,
            allow_cache=(not environ.honey_build_cache_skip_profiler()),
        )
...
def get_compile_engine(n_cpus: int = -1):
    if is_cmake_compilation():
        from honey.backend.cuda import builder_cmake
