...
class {{model_name}} : public ModelBase<{{model_name}}> {
  ...
  public:
    ...
    if (workspace_type == HoneyWorkspaceAllocationMode::kEager) {
      SetUpWorkspace();
    }
...
{% if run_impl_mode == 0 %}
    ...
    void RunImpl(StreamType stream) {
        ...
        RAII_ProfilerRange _raiiHoneyProfilerRange("main_start");
        {% endif %}
...
{% endif %}
    ...
    void ProfileImpl(StreamType stream, size_t iters, const std::string& filename) {
      ...
      std::cout << "Honey per op profiling finished." << std::endl;
...
{{ set_up_output_shapes }}
  for (size_t i = 0; i < num_params_; ++i) {
    ...
    max_param_storage_bytes_[i] = max_param_numel_[i] * HoneyDtypeSizeBytes(param_dtypes_[i]);
  }
...
ModelContainer* CreateModelContainer(size_t num_runtimes, HoneyAllocator& allocator) {
