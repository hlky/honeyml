...
class Transformer2DModel(nn.Module):
    ...
    def _operate_on_continuous_inputs(self, hidden_states):
        batch, height, width, _ = ops.size()(hidden_states)
    ...
    def _get_output_for_continuous_inputs(
        ...
        if not self.use_linear_projection:
            hidden_states = ops.reshape()(
                hidden_states, [batch_size, height, width, inner_dim]
            )
    ...
    def _get_output_for_vectorized_inputs(self, hidden_states):
        ...
        # log(p(x_0))
        output = ops.log(ops.softmax()(logits))
        return output
    ...
    def _get_output_for_patched_inputs(
        ...
        if self.norm_type != "ada_norm_single":
            ...
            shift, scale = ops.chunk()(
                self.proj_out_1(ops.silu(conditioning)), 2, dim=1
            )
            hidden_states = self.norm_out(hidden_states) * (
