...
class AutoencoderTinyOutput(BaseOutput):
    ...
    Args:
        latents (`Tensor`): Encoded outputs of the `Encoder`.
...
class AutoencoderTiny(nn.Module):
    r"""
    ...
    def _tiled_encode(self, x: Tensor) -> Tensor:
        ...
        Returns:
            `Tensor`: Encoded batch of images.
        """
        ...
        # output array
        out = torch.zeros(
            x.shape[0], 4, x.shape[-2] // sf, x.shape[-1] // sf, device=x.device
        )
        for i in ti:
            for j in tj:
                ...
                blend_mask_i = (
                    torch.ones_like(blend_masks[0]) if i == 0 else blend_masks[0]
                )
                blend_mask_j = (
                    torch.ones_like(blend_masks[1]) if j == 0 else blend_masks[1]
                )
    ...
    def _tiled_decode(self, x: Tensor) -> Tensor:
        ...
        # output array
        out = torch.zeros(
            x.shape[0], 3, x.shape[-2] * sf, x.shape[-1] * sf, device=x.device
        )
        for i in ti:
            for j in tj:
                tile_in = x[..., i : i + tile_size, j : j + tile_size]
                # tile result
                tile_out = out[
                    ..., i * sf : (i + tile_size) * sf, j * sf : (j + tile_size) * sf
                ]
