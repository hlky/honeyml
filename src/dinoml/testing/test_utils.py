...
from dinoml.frontend.nn.module import Module as DinoMLModule
from dinoml.testing.benchmark_pt import benchmark_torch_function
from dinoml.testing.detect_target import detect_target
...
def benchmark_module(
    ...
    graph_mode = False
    t, _, __ = dinoml_mod.benchmark_with_tensors(
        [inputs],
        [outputs],
        count=iters,
    ...
    # benchmark
    t_dinoml, _, __ = dinoml_mod.benchmark_with_tensors(
        [inputs],
        [outputs],
        count=iters,
        graph_mode=graph_mode,
    ...
    print(
        f"{name}_dinoml: batch_size: {batch_size}, {t_dinoml} ms",
    )
