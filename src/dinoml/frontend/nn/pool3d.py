...
from dinoml.compiler.ops import max_pool2d
from dinoml.compiler.ops.common import reshape
...
def identical_elem_tuple_to_int(param):
    ...
    if not isinstance(param, (list, tuple)) or not all(x == param[0] for x in param):
        raise RuntimeError(f"DinoML supports square param values only, but got {param}")
    return param[0]
