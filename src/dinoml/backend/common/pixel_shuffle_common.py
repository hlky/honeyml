...
__global__ void pixel_shuffle_kernel(
    ...
    {{index_type}} total_elements = batch * in_h * in_w * in_ch * r * r;
    if (idx < total_elements) {
        {{index_type}} out_ch = in_ch / (r * r);
        {{index_type}} n = idx / (in_h * in_w * out_ch * r * r);
        {{index_type}} c = (idx / (in_h * in_w * r * r)) % out_ch;
        {{index_type}} h = (idx / (in_w * r * r)) % in_h;
        {{index_type}} w = (idx / (r * r)) % in_w;
        {{index_type}} rh = (idx / r) % r;
        {{index_type}} rw = idx % r;

        {{index_type}} in_index = n * in_h * in_w * in_ch + h * in_w * in_ch + w * in_ch + c * r * r + rh * r + rw;
        {{index_type}} out_index = n * (in_h * r) * (in_w * r) * out_ch + (h * r + rh) * (in_w * r) * out_ch + (w * r + rw) * out_ch + c;
...
void {{function_name}}(
    ...
    pixel_shuffle_kernel<<<num_blocks, threads_per_block, 0, stream>>>(
