...
#include <ops/kupsample2d_weight.h>
...
void {{func_name}}(
    ...
    invoke_kupsample2d_weight<{{dtype}}>(out, channels, stream);
}
"""
)
...
@registry.reg("cuda.kupsample2d_weight.gen_function")
def cuda_kupsample2d_weight_gen_function(attrs, indent="  "):
    return gen_function(attrs, CUDASpec())


@registry.reg("cuda.kupsample2d_weight.func_decl")
def cuda_kupsample2d_weight_gen_decl(attrs):
    return FUNC_DECL.render(func_name=attrs["name"])


@registry.reg("cuda.kupsample2d_weight.func_call")
def cuda_kupsample2d_weight_gen_function_call(attrs, indent="  "):
