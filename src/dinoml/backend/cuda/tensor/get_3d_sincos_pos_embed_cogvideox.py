...
void {{function_name}}(
    ...
    dinoml::invoke_sincos_pos_embed_3d_cogvideox_joint<{{elem_output_type}}>(
        out,
        embed_dim,
        spatial_w,
        spatial_h,
...
def gen_function_call(func_attrs: Dict[str, Any], indent="  ") -> str:
    ...
    max_text_seq_length = _to_int_expr(func_attrs["max_text_seq_length"])
...
@registry.reg("cuda.get_3d_sincos_pos_embed_cogvideox.gen_function")
def cuda_get_3d_sincos_pos_embed_cogvideox_gen_function(func_attrs):
    return gen_function(func_attrs, CUDASpec())


@registry.reg("cuda.get_3d_sincos_pos_embed_cogvideox.func_decl")
def cuda_get_3d_sincos_pos_embed_cogvideox_gen_function_decl(
    func_attrs: Dict[str, Any],
) -> str:
    return gen_function_decl(func_attrs, CUDASpec())


@registry.reg("cuda.get_3d_sincos_pos_embed_cogvideox.func_call")
def cuda_get_3d_sincos_pos_embed_cogvideox_func_call(func_attrs, indent="  "):
