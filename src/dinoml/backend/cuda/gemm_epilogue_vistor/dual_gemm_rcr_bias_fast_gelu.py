...
from dinoml.backend.cuda.gemm_epilogue_vistor import common_dual_gemm_bias
from dinoml.backend.cuda.gemm_universal import common, common_bias
from dinoml.backend.cuda.gemm_universal.layout import RCR
...
PROBLEM_ARGS_TEMPLATE = jinja2.Template(
    ...
    {({{elem_input_type}}*)b0_ptr, LayoutB(K)},     // TensorRef<ElementB const, LayoutB0> ref_B0_
    {({{elem_input_type}}*)bias0_ptr, 0},           // TensorRef<ElementC const, LayoutC> ref_C0_
    nullptr_ref,                                    // TensorRef<ElementC, LayoutC> ref_D0_
{% if broadcast_b1 %}
    {({{elem_input_type}}*)b1_ptr, 0},              // TensorRef<ElementB const, LayoutB1> ref_B1_
{% else %}
    {({{elem_input_type}}*)b1_ptr, LayoutB(K)},     // TensorRef<ElementB const, LayoutB1> ref_B1_
{% endif %}
    {({{elem_input_type}}*)bias1_ptr, 0},           // TensorRef<ElementC const, LayoutC> ref_C1_
    nullptr_ref,                                    // TensorRef<ElementC, LayoutC> ref_D1_
    {({{elem_output_type}}*)c_ptr, LayoutC(N)},     // TensorRef<ElementC, LayoutC> ref_D2_
    {ElementCompute(1), ElementCompute(1)},         // typename EpilogueOutputOp0::Params epilogue0_
    {ElementCompute(1), ElementCompute(1)},         // typename EpilogueOutputOp1::Params epilogue1_
...
@registry.reg("cuda.dual_gemm_rcr_bias_fast_gelu.config")
def gemm_rcr_config(func_attrs, dtype="float16"):
    common_dual_gemm_bias.make_fproc(
        func_attrs,
        RCR,
        dtype=dtype,
    )
...
def common_gen_profiler(
    ...
    return common_dual_gemm_bias.gen_profiler(
        func_attrs,
        workdir,
        profiler_filename,
        dim_info_dict,
...
@registry.reg("cuda.dual_gemm_rcr_bias_fast_gelu.gen_profiler")
def gen_profiler(func_attrs, workdir, profiler_filename, dim_info_dict):
    ...
    return common_gen_profiler(
        ...
        profiler_filename,
        dim_info_dict,
        common_dual_gemm_bias.SRC_TEMPLATE,
...
@registry.reg("cuda.dual_gemm_rcr_bias_fast_gelu.gen_function")
def gen_function(
    func_attrs,
    exec_cond_template,
    dim_info_dict,
    ...
    return common_dual_gemm_bias.gen_function(
        func_attrs,
        exec_cond_template,
        problem_args,
        input_ndims,
...
@registry.reg("cuda.dual_gemm_rcr_bias_fast_gelu.func_decl")
def gen_function_decl(func_attrs):
    func_name = func_attrs["name"]
    input_ndims = len(func_attrs["input_accessors"][0].original_shapes)
    weight_ndims = len(func_attrs["input_accessors"][1].original_shapes)
    return common_dual_gemm_bias.FUNC_DECL_TEMPLATE.render(
        func_name=func_name,
...
@registry.reg("cuda.dual_gemm_rcr_bias_fast_gelu.func_call")
def gen_function_call(func_attrs, indent="  "):
    return common_dual_gemm_bias.gen_function_call(func_attrs, indent)


@registry.reg("cuda.dual_gemm_rcr_bias_fast_gelu.filter")
