...
from dinoml.utils.misc import is_linux
...
DINOML_STATIC_FILES_PATH = str(ROOT_PATH.joinpath("csrc/"))
...
class Target:
    def __init__(self, static_files_path: str):
        ...
        ndebug_str = os.getenv("DINOML_NDEBUG", "1")
        try:
    ...
    def _prepare_profile_cache_path(self) -> Optional[str]:
        ...
        if prefix is None:
            prefix = os.path.join(pathlib.Path.home(), ".dinoml")

        try:
            os.makedirs(prefix, exist_ok=True)
        except OSError as error:
            _LOGGER.info(f"Cannot mkdir at {prefix} due to issue {error}")
            prefix = os.path.join(tempfile.mkdtemp(prefix="dinoml_"), ".dinoml")
            os.makedirs(prefix, exist_ok=True)
