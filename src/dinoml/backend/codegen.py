...
from dinoml.backend.main_templates import MODEL_CONTAINER_TEMPLATE, MODEL_TEMPLATE
from dinoml.backend.target import Target

from dinoml.compiler.base import IntImm, IntVar, IntVarTensor, Operator, Tensor
from dinoml.compiler.dtype import dtype_to_enumerator, get_dtype_size
from dinoml.compiler.tensor_accessor import TensorAccessor

from dinoml.compiler.transform.memory_planning import Workspace
from dinoml.compiler.workspace import workspace_mode, WorkspaceAllocationMode
from dinoml.utils.debug_settings import DinoMLDebugSettings
from dinoml.utils.environ import (
    allocation_mode,
...
from dinoml.utils.misc import is_debug, is_windows
...
def gen_profiler(sorted_graph: List[Tensor], workdir: str, dynamic_profiling_strategy):
    ...
    dynamic_profiling_strategy: DynamicProfileStrategy, optional
        ...
        See also: :func:`~dinoml.compiler.transform.profile.profile`
    """
    results = []
...
_DEBUG_SETTINGS = DinoMLDebugSettings()
...
def gen_library_src(  # noqa: C901
    ...
    debug_settings : DinoMLDebugSettings
        specify debug settings such as where to dump DinoML model Python file, etc.
