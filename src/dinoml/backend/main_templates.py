...
class {{model_name}} : public ModelBase<{{model_name}}> {
  ...
  public:
    ...
    if (workspace_type == DinoMLWorkspaceAllocationMode::kEager) {
      SetUpWorkspace();
    }
...
{% if run_impl_mode == 0 %}
    ...
    void RunImpl(StreamType stream) {
        ...
        RAII_ProfilerRange _raiiDinoMLProfilerRange("main_start");
        {% endif %}
...
{% endif %}
    ...
    void ProfileImpl(StreamType stream, size_t iters, const std::string& filename) {
      ...
      std::cout << "DinoML per op profiling finished." << std::endl;
...
{{ set_up_output_shapes }}
  for (size_t i = 0; i < num_params_; ++i) {
    ...
    max_param_storage_bytes_[i] = max_param_numel_[i] * DinoMLDtypeSizeBytes(param_dtypes_[i]);
  }
