...
#
"""
A common place for holding DinoML-related env control variables
"""
...
def get_compiler_opt_level() -> str:
    ...
    compiler_opt = os.getenv("DINOML_COMPILER_OPT", "-O1")
...
def enable_cuda_lto() -> bool:
    ...
    Default value is "0".
    """
    return os.getenv("DINOML_ENABLE_CUDA_LTO", "0") == "1"
...
def nvcc_ccbin() -> str:
    ...
    """
    return os.getenv("DINOML_NVCC_CCBIN", "")
...
def force_profiler_cache() -> bool:
    ...
    """
    force_cache = os.environ.get("DINOML_FORCE_PROFILER_CACHE", None) == "1"
    if force_cache:
        assert os.environ.get("FORCE_PROFILE", None) != "1", (
            "cannot specify both DINOML_FORCE_PROFILER_CACHE and FORCE_PROFILE"
        )
    return force_cache
...
def time_compilation() -> bool:
    ...
    """
    return os.getenv("DINOML_TIME_COMPILATION", "0") == "1"
...
def shorten_tensor_names_for_plots() -> bool:
    ...
    """
    return os.getenv("DINOML_PLOT_SHORTEN_TENSOR_NAMES", "0") == "1"


def dinoml_build_cache_dir() -> Optional[str]:
    """
    When set to a non-empty string, cache the build artifacts
    below this directory for significantly faster builds.

    See dinoml.backend.build_cache

    Returns:
        Optional[str]: Value of DINOML_BUILD_CACHE_DIR environment variable,
        or None if not set.
    """
    return os.environ.get("DINOML_BUILD_CACHE_DIR", None)


def dinoml_build_cache_skip_percentage() -> int:
    """
    When set to a non-empty string, and if DINOML_BUILD_CACHE_DIR
    is set, the build cache will be skipped randomly with
    a probability correspinding to the specified percentage

    Returns:
        int: Integer value of DINOML_BUILD_CACHE_SKIP_PERCENTAGE environment variable,
        or 5 if not set.
    """
    return int(os.environ.get("DINOML_BUILD_CACHE_SKIP_PERCENTAGE", "30"))


def dinoml_build_cache_skip_profiler() -> bool:
    """
    boolean value of DINOML_BUILD_CACHE_SKIP_PROFILER environment variable.
    ...
    in all other cases.
    """
    ret = os.environ.get("DINOML_BUILD_CACHE_SKIP_PROFILER", "1")
    if ret is None or ret == "" or ret == "0" or ret.lower() == "false":
        return False
    return True


def dinoml_build_cache_max_mb() -> int:
    """
    boolean value of DINOML_BUILD_CACHE_MAX_MB environment variable.
    ...
    """
    return int(os.environ.get("DINOML_BUILD_CACHE_MAX_MB", "30"))
...
def allow_cutlass_sm90_kernels() -> bool:
    ...
    return (
        ...
        or os.getenv("DINOML_ALLOW_CUTLASS_SM90_KERNELS", "0") == "1"
    )
...
def force_cutlass_sm90_kernels() -> bool:
    ...
    """
    return os.getenv("DINOML_FORCE_CUTLASS_SM90_KERNELS", "0") == "1"
...
def multistream_mode() -> int:
    ...
    return int(os.getenv("DINOML_MULTISTREAM_MODE", "0"))
...
def multistream_additional_streams() -> int:
    ...
    This option is independent from DINOML_MULTISTREAM_MAX_MEM_PARALLEL_OPS.

    For example, say, there are 100 ops that can be run in parallel.

    Example 1: DINOML_MULTISTREAM_EXTRA_STREAMS=4 and DINOML_MULTISTREAM_MAX_MEM_PARALLEL_OPS=100.
    ...
    Example 2: DINOML_MULTISTREAM_EXTRA_STREAMS=4 and DINOML_MULTISTREAM_MAX_MEM_PARALLEL_OPS=5.
    In this case 5 streams will be used (1 base and 4 extra),
    ...
    """
    return int(os.getenv("DINOML_MULTISTREAM_EXTRA_STREAMS", "4"))
...
def multistream_max_mem_parallel_ops() -> int:
    ...
    This option is independent from DINOML_MULTISTREAM_EXTRA_STREAMS.
    ...
    return int(os.getenv("DINOML_MULTISTREAM_MAX_MEM_PARALLEL_OPS", "99999999"))
...
def allocation_mode() -> int:
    ...
    Default: 0.
    """
    return int(os.getenv("DINOML_ALLOCATION_MODE", "0"))
...
def enable_standalone_exe_generation() -> bool:
    """
    Whether to generate standalone binaries for DinoML build directories.
    Defaults to False.
    """
    return os.getenv("DINOML_ENABLE_STANDALONE", "0") == "1"
...
def enable_ptxas_info():
    ...
    """
    return os.getenv("DINOML_ENABLE_PTXAS_INFO", "0") == "1"


def get_cuda_nvcc_debug_level():
    """
    Return level of CUDA debug information. Default to no debug info.
    backed by env var DINOML_CUDA_DEBUG_LEVEL. Which may either be a string
    ...
    """
    level = os.getenv("DINOML_CUDA_DEBUG_LEVEL", "0")
    return level
