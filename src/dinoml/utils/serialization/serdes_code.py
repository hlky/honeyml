...
from dinoml.compiler.transform import mark_param_tensor, name_graph, toposort
...
from dinoml.compiler.base import IntImm, IntVar, _HostConstantTensorData, _NumpyConstantTensorData
from dinoml.compiler.ops.common.epilogue import FuncEnum
from dinoml.frontend import Tensor

from dinoml.utils.serialization.dinoml_program import DinoMLBasicProgram

class DinoMLProgram(DinoMLBasicProgram):
...
def dump_program(
    ...
    """This function dumps out an DinoML sorted graph to an executable python code.

    Parameters
    ----------
    sorted_graph : Union[Tensor, List[Tensor]]
        Final tensor(s) that are associated to the DinoML graph.
    file_path: str
...
def _get_class(file_path: str, class_name: str = "DinoMLProgram"):
    import importlib.util

    spec = importlib.util.spec_from_file_location("DinoMLProgram", file_path)
