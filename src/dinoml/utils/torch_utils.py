...
DinoML doesn't depend on PyTorch, but it exposes
...
from dinoml.compiler.dtype import dtype_str_to_enum, get_dtype_size, normalize_dtype
...
def torch_dtype_to_string(dtype):
    for torch_dtype, dinoml_dtype in types_mapping():
...
def string_to_torch_dtype(string_dtype):
    ...
    for torch_dtype, dinoml_dtype in types_mapping():
        if string_dtype == dinoml_dtype:
            return torch_dtype
    raise ValueError(
        f"Got unsupported dinoml dtype {string_dtype}! "
        f"Supported dtypes are: {list(types_mapping())}"
    )
