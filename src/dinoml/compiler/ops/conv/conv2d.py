...
from dinoml.compiler.ops.conv.cache_entry import ConvQueryEntry, ConvRecordEntry
from dinoml.compiler.ops.conv.conv_common import (
    filter_op_instances,
    generate_profiler_sources,
    get_profiler_filename,
)
from dinoml.compiler.ops.padding import nhwc3to4, nhwc3to8
from dinoml.utils import alignment, environ, shape_utils
...
class conv2d(Operator):
    ...
    * :attr:`group` controls the number of blocked connections from input channels
    ...
    .. code-block:: python
        ...
        OP = dinoml.compiler.ops.conv2d(stride=1, pad=1, dilate=1)
        Y = OP(X, W)

    .. highlight:: python
    .. code-block:: python

        X_pt = NHWC2NCHW(X_dinoml)
        W_pt = NHWC2NCHW(W_dinoml)
