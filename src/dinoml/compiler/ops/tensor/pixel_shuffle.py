...
class pixel_shuffle(Operator):
    """
    Rearranges elements in a tensor of shape [N, H, W, C * r^2] to a tensor of shape [N, H * r, W * r, C].

    * :attr:`r` (int): upscale factor.

    Args:
        input (Tensor [N, H, W, C * r^2]): the input data.
    ...
    def _signature(self):
        signature = "pixel_shuffle: r=[{r}]".format(r=self._attrs["r"])
        return signature
