...
from dinoml.compiler.ops.gemm_special.gemm_rrr_small_nk import gemm_rrr_small_nk
from dinoml.compiler.ops.gemm_universal.bmm_xxx import bmm_rcr
...
from dinoml.utils.shape_utils import is_singleton_dimension
...
def _simple_transform_with_constraint(
    ...
    ----------
    List[Tensor]
        DinoML graph after transformation
    """
...
def _transform_bmm_rcr_n1(sorted_graph: List[Tensor]) -> List[Tensor]:
    ...
    ----------
    List[Tensor]
        DinoML graph with suitable bmm_rcr substituted with bmm_rcr_n1
    """
...
def _transform_gemm_rrr_small_nk(sorted_graph: List[Tensor]) -> List[Tensor]:
    ...
    ----------
    List[Tensor]
        DinoML graph with suitable gemm_rrr substituted with gemm_rrr_small_nk
    """
    matchFunc, replaceFunc = _single_source_shape_constraint_funcs(
