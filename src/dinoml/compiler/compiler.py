...
from dinoml import backend, compiler
...
from dinoml.compiler.model import (
    DINOML_DEFAULT_NUM_RUNTIMES,
    DinoMLAllocatorKind,
    Model,
    TorchTensor,
)
from dinoml.compiler.transform.name_graph import reset_name_counters
from dinoml.compiler.transform.profile import elapsed_dt_sec
from dinoml.utils import graph_utils
from dinoml.utils.debug_settings import DinoMLDebugSettings
from dinoml.utils.misc import callstack_stats
from dinoml.utils.serialization.serdes_code import dump_program
...
def compile_model(
    ...
    recompile = os.getenv("DINOML_RECOMPILE", "1")
    graph = None
    ...
    _LOGGER.info(f"Start to compile DinoML model. {test_dir=}")
    ...
    if int(recompile) == 1:
        ...
        with target:
            ...
            if debug_settings.dump_dinoml_to_py:
                dump_program(tensor, debug_settings.dump_dinoml_to_py)
