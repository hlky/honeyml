...
from dinoml.compiler import symbolic
from dinoml.compiler.dtype import get_dtype_size, normalize_dtype
from dinoml.compiler.op_registry import OP_REGISTRY

from dinoml.compiler.stable_set import StableSet

from dinoml.utils.tensor_utils import wrap_dim
...
class JaggedIntVar(IntVar):
    ...
    Because JaggedIntVar is an IntVar, it can be treated so by the DinoML ops
    ...
    def offsets_struct_type(self) -> str:
        ...
        return f"dinoml::JaggedOffsets<{self.offsets_type()}, {num_jagged_dims}>"
...
class Tensor(Node):
    ...
    def __init__(
        ...
        original_name : str, optional
            Original name of this tensor before making it DinoML friendly
        """
        super().__init__()
