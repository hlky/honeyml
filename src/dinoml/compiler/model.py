...
from dinoml.compiler.dtype import dtype_str_to_enum
from dinoml.utils.misc import is_linux, is_windows
from dinoml.utils.torch_utils import torch_dtype_to_string, write_tensor_binary
...
class DinoMLMemcpyKind(enum.Enum):
    HostToDevice = 0
    DeviceToHost = 1
    DeviceToDevice = 2


class DinoMLAllocatorKind(enum.Enum):
    DEFAULT = 0
    TRACKING = 1


class DinoMLData(NamedTuple):
    """
...
class _DinoMLShape(ctypes.Structure):
    _fields_ = [
...
class _CFormatDinoMLData(ctypes.Structure):
    _fields_ = [
...
def torch_to_dinoml_data(tensor: TorchTensor) -> DinoMLData:
    """
...
def _convert_tensor_args(params: Union[List[TorchTensor], Dict[str, TorchTensor]]):
    ...
    if isinstance(params, dict):
        result = {name: torch_to_dinoml_data(x) for name, x in params.items()}
    else:
        result = [torch_to_dinoml_data(x) for x in params]
    return result
...
class Model:
    class _DLLWrapper:
        ...
        def __getattr__(self, name):
            if not self.is_open:
                raise RuntimeError(f"Cannot use closed DinoML library: {self.lib_path}")
    ...
    def __init__(
        self,
        lib_path: str,
        num_runtimes: int = DINOML_DEFAULT_NUM_RUNTIMES,
        allocator_kind: Optional[DinoMLAllocatorKind] = None,
    ):
        """
        ...
        allocator_kind : DinoMLAllocatorKind, optional
            What type of allocator to use when allocating GPU memory.
        """
        # Set of pointers allocated with numpy_to_dinoml_data.
        # If the user forgets to free their data, we use this to
        # avoid leaking memory.
        self._allocated_dinoml_data = set()
    ...
    def close(self):
        ...
        for ptr in list(self._allocated_dinoml_data):
        ...
        if hasattr(self, "DLL"):
            if self.handle:
                self.DLL.DinoMLModelContainerDelete(self.handle)
                self.handle = ctypes.c_void_p()

            if self.allocator_handle:
                self.DLL.DinoMLAllocatorDelete(self.allocator_handle)
                self.allocator_handle = ctypes.c_void_p()
    ...
    def _convert_single_param_to_c_format(
        ...
        c_shape = _DinoMLShape(c_shape_data, ctypes.c_size_t(len(shape)))
    ...
    def _make_dinoml_outputs(
        self, outputs: List[DinoMLData], c_output_shapes
    ) -> Dict[str, DinoMLData]:
        output_shapes = []
    ...
    def _run_impl(
        self,
        inputs: Union[Dict[str, DinoMLData], List[DinoMLData]],
        outputs: Union[Dict[str, DinoMLData], List[DinoMLData]],
        ...
        if not outputs_on_host:
            self.DLL.DinoMLModelContainerRun(
                self.handle,
                c_inputs,
            ...
            self.DLL.DinoMLModelContainerRunWithOutputsOnHost(
                self.handle,
                c_inputs,
                ctypes.c_size_t(len(inputs)),
                c_outputs,
        ...
        return self._make_dinoml_outputs(outputs, c_output_shapes_out)
    ...
    def run(
        ...
        ----------
        inputs: Union[Dict[str, DinoMLData], List[DinoMLData]]
            The inputs to use. DinoMLData is a named tuple containing
            ...
            This parameter can also be a dictionary (name -> DinoMLData).
        outputs: Union[Dict[str, DinoMLData], List[DinoMLData]]
            The outputs to use. Similar to inputs, can either be a list of ordered
            outputs, or a dictionary (output name -> DinoMLData).
        ...
        Returns
        -------
        DinoMLDatas with output shapes that are computed by shape inference. This may not be
    ...
    def profile(
        ...
        self.DLL.DinoMLModelContainerProfile(
            self.handle,
            c_inputs,
            ctypes.c_size_t(len(inputs)),
            c_outputs,
    ...
    def _interpret_tensors_as_shapes(
        self,
        outputs: Union[List[TorchTensor], Dict[str, TorchTensor]],
        outputs_dinoml: Dict[str, DinoMLData],
    ) -> Dict[str, TorchTensor]:
        if isinstance(outputs, dict):
            return {
                name: _reshape_tensor(tensor, outputs_dinoml[name].shape)
                for name, tensor in outputs.items()
            }
        else:
            return {
                name: _reshape_tensor(outputs[idx], outputs_dinoml[name].shape)
                for name, idx in self._output_name_to_index.items()
            }
    ...
    def benchmark(
    ...
    ) -> Tuple[float, float, Dict[str, DinoMLData]]:
        """
        ...
        for _ in range(repeat):
            self.DLL.DinoMLModelContainerBenchmark(
                self.handle,
                c_inputs,
                ctypes.c_size_t(len(inputs)),
                c_outputs,
        ...
        return (mean, std, self._make_dinoml_outputs(outputs, c_output_shapes_out))
    ...
    def get_required_memory(self):
        ...
        self.DLL.DinoMLModelContainerGetRequiredMemory(
    ...
    def _construct_output_name_to_index_map(self) -> Dict[str, int]:
        ...
        self.DLL.DinoMLModelContainerGetNumOutputs(
    ...
    def set_constant(self, name: str, tensor: DinoMLData):
        """
        ...
        self.DLL.DinoMLModelContainerSetConstant(
            self.handle, c_name, ctypes.byref(c_tensor)
        )

    def set_many_constants(self, tensors: Dict[str, DinoMLData]):
        """
    ...
    def set_double_buffer_constant(
        ...
        c_name = ctypes.c_char_p(b_name)
        c_tensor = self._convert_single_param_to_c_format(tensor)
        self.DLL.DinoMLModelContainerSetDoubleBufferConstant(
            self.handle, ctypes.c_void_p(stream_ptr), c_name, ctypes.byref(c_tensor)
        )

    def set_many_double_buffer_constants(
        self, tensors: Dict[str, DinoMLData], stream_ptr: Optional[int] = None
    ):
        """
        Bulk set many constants at once. More efficient than set_constant()
        since it only has to acquire the lock once.
        ...
        num_tensors = ctypes.c_size_t(len(tensors))
        self.DLL.DinoMLModelContainerSetManyDoubleBufferConstants(
    ...
    def set_many_constants_with_tensors(self, tensors: Dict[str, TorchTensor]):
        ...
        for name, tensor in tensors.items():
            ...
            dinoml_tensors[name] = torch_to_dinoml_data(tensor)
        self.set_many_constants(dinoml_tensors)
    ...
    def set_double_buffer_constant_with_tensor(
        ...
        if not tensor.is_contiguous() or not tensor.is_cuda:
            raise ValueError(f"Constant {name} must be contiguous and on the GPU.")
        self.torch_constant_tensors[name] = tensor
        self.set_double_buffer_constant(name, torch_to_dinoml_data(tensor), stream_ptr)
    ...
    def set_many_double_buffer_constants_with_tensors(
        ...
        for name, tensor in tensors.items():
            ...
            self.torch_constant_tensors[name] = tensor
            dinoml_tensors[name] = torch_to_dinoml_data(tensor)
        self.set_many_double_buffer_constants(dinoml_tensors, stream_ptr)
    ...
    def set_constant_with_tensor(self, name: str, tensor: TorchTensor):
        ...
        if not tensor.is_contiguous() or not tensor.is_cuda:
            raise ValueError(f"Constant {name} must be contiguous and on the GPU.")
        self.torch_constant_tensors[name] = tensor
        self.set_constant(name, torch_to_dinoml_data(tensor))
    ...
    def get_input_maximum_shape(self, input_idx_or_name: Union[int, str]) -> List[int]:
        ...
        raw_shape = Shape()
        self.DLL.DinoMLModelContainerGetMaximumInputShape(
    ...
    def get_input_dtype(self, index):
        ...
        """
        input = ctypes.c_int()
        self.DLL.DinoMLModelContainerGetInputDtype(
    ...
    def get_output_maximum_shape(
        ...
        raw_shape = Shape()
        self.DLL.DinoMLModelContainerGetMaximumOutputShape(
    ...
    def get_output_dtype(self, index):
        ...
        """
        output = ctypes.c_int()
        self.DLL.DinoMLModelContainerGetOutputDtype(
    ...
    def free_gpu_memory(
        ...
        third-party libraries like PyTorch or pycuda are not available.
        """
        if ptr in self._allocated_dinoml_data:
            self._allocated_dinoml_data.remove(ptr)

        self.DLL.DinoMLDeviceFree(
    ...
    def memcpy(
        ...
        specified by the DinoMLMemcpyKind enum.
        """
        self.DLL.DinoMLMemcpy(
            ctypes.c_void_p(dst),
            ctypes.c_void_p(src),
            ctypes.c_size_t(count),
    ...
    def get_num_runtimes(self) -> int:
        ...
        """
        out = ctypes.c_size_t()
        self.DLL.DinoMLModelContainerGetNumRuntimes(self.handle, ctypes.byref(out))
        return out.value

    def numpy_to_dinoml_data(
        self, arr: np.ndarray, stream_ptr: Optional[int] = None, sync: bool = True
    ) -> DinoMLData:
        """
        Convert a numpy array to DinoML-usable data. Mallocs and copies
        on the given stream.
        ...
        self._allocated_dinoml_data.add(gpu_mem)
        self.memcpy(
            gpu_mem,
        ...
        return DinoMLData(gpu_mem, shape, dtype)

    def dinoml_data_to_numpy(
        self,
        dinoml_data: DinoMLData,
        stream_ptr: Optional[int] = None,
        sync: bool = True,
    ) -> np.ndarray:
        """
        Create numpy array from an DinoMLData.
        Copies on the given stream.
        """
        arr = np.empty(dinoml_data.shape, dtype=dinoml_data.dtype)
        self.memcpy(
            arr.ctypes._data.value,
            dinoml_data.data_ptr,
            arr.nbytes,
    ...
    def fold_constants(
        ...
        if double_buffer:
            self.DLL.DinoMLModelContainerFoldConstantsInDoubleBuffer(
                self.handle,
                ctypes.c_void_p(stream_ptr),
                ctypes.c_bool(sync),
            )
        else:
            self.DLL.DinoMLModelContainerFoldConstants(
                self.handle,
                ctypes.c_void_p(stream_ptr),
                ctypes.c_bool(sync),
            )
    ...
    def swap_constants(self):
        self.DLL.DinoMLModelContainerSwapConstants(self.handle)
    ...
    def _get_constant_names_impl(
        ...
        self.DLL.DinoMLModelContainerGetNumConstants(
            self.handle,
        ...
        self.DLL.DinoMLModelContainerGetConstantNames(
