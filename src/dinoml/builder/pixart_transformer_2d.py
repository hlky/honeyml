...
from dinoml.mapping.pixart_transformer_2d import map_pixart_transformer_2d
...
def build(
    ...
    **kwargs,
    ...
    if isinstance(dtype, torch.dtype):
        dinoml_dtype = torch_dtype_to_string(dtype)
    else:
        dinoml_dtype = dtype
    config, dinoml_cls, pt_cls = load_config(hf_hub, **kwargs)
    dinoml_module = cast(nn.Module, dinoml_cls(**config, dtype=dinoml_dtype))
