...
class GEMMTestCase(unittest.TestCase):
    ...
    def _test_h_rcr(self, honey_dtype, test_name=None):
        if test_name is None:
            test_name = honey_dtype

        M = 256
        K = 256
        N = 512
        target = detect_target(use_fp16_acc=(honey_dtype == "float16"))
        X = Tensor(shape=[M, K], dtype=honey_dtype, name="input_0", is_input=True)
        W = Tensor(shape=[N, K], dtype=honey_dtype, name="input_1", is_input=True)
        OP = ops.gemm_rcr()
        Y = OP(X, W)
        Y._attrs["name"] = "output_0"
        Y._attrs["is_output"] = True
        ...
        self._test_id += 1
        X_pt = get_random_torch_tensor((M, K), honey_dtype)
        W_pt = get_random_torch_tensor((N, K), honey_dtype)
        Y_pt = torch.nn.functional.linear(X_pt, W_pt)

        inputs = {"input_0": X_pt, "input_1": W_pt}
        y = get_torch_empty_tensor((M, N), honey_dtype)
        module.run_with_tensors(inputs, [y])
        torch.testing.assert_close(Y_pt, y, atol=1e-1, rtol=1e-1)

    def test_h_rcr_float16(self):
        self._test_h_rcr(honey_dtype="float16")

    def test_h_rcr_float16_rocm(self):
        self._test_h_rcr(honey_dtype="float16", test_name="float16_rocm")

    def test_h_rcr_float32_sm80(self):
        self._test_h_rcr(honey_dtype="float32")

    def test_h_rcr_bfloat16_bf16(self):
        self._test_h_rcr(honey_dtype="bfloat16")
